...
export default function TuningChat({ messages, isLoading, hasError, onSend }: TuningChatProps) {
    ...
    <div className="border-t border-gray-200 pt-4 mt-4">
        ...
        <div className="space-y-2 mb-3 max-h-48 overflow-y-auto">
          {messages.map((msg, i) => (
            ...
            >
              ...
              >
                {msg.role === 'assistant' ? `已完成：${msg.content.slice(0, 60)}…` : msg.content}
              </div>
            </div>
          ))}
          {isLoading && (
      ...
      <div className="flex gap-2 items-end">
        <textarea
          ...
          placeholder="输入修改指令，例如：把开场白改得更亲切（Shift+Enter 换行）"
          disabled={isLoading}
          rows={1}
